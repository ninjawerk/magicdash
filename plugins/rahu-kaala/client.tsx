...
function RahuKaalaWidget({ config, context, size, setAlert, editMode, openSettings }: WidgetProps<Config>) {
  ...
  const extras: Period[] = [
    ...(config.showYamagandam ? today.periods.filter((p) => p.key === 'yamagandam') : []),
    ...(config.showGulika ? today.periods.filter((p) => p.key === 'gulika') : []),
  ];
        ...
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-white/10 pt-2 text-xs text-white/55">
          ...
          {config.showSun !== false && (
            <span className="ml-auto flex items-center gap-2">
              <Sunrise size={12} className="text-[var(--warm)]" /> {fmt(today.sunrise)}
              <Sunset size={12} className="text-[var(--accent)]" /> {fmt(today.sunset)}
            </span>
          )}
        </div>
      )}
...
export default definePlugin<Config>({
  ...
  translations: {
    ...
    ta: { rahu: 'ராகு காலம்', yamagandam: 'எமகண்டம்', gulika: 'குளிகை', today: 'இன்று', tomorrow: 'நாளை', now: 'இப்போது ராகு காலம்', soon: 'விரைவில்', over: 'இன்று முடிந்தது', startsIn: 'தொடங்க', endsIn: 'முடிய' },
    si: { rahu: 'රාහු කාලය', yamagandam: 'යමගණ්ඩ', gulika: 'ගුලික', today: 'අද', tomorrow: 'හෙට', now: 'දැන් රාහු කාලය', soon: 'ළඟදීම', over: 'අද අවසන්', startsIn: 'ආරම්භයට', endsIn: 'අවසානයට' },
    hi: { rahu: 'राहु काल', yamagandam: 'यमगण्ड', gulika: 'गुलिक', today: 'आज', tomorrow: 'कल', now: 'अभी राहु काल', soon: 'जल्द शुरू', over: 'आज समाप्त', startsIn: 'शुरू होने में', endsIn: 'समाप्त होने में' },
  },
});
